				...
				<div class="mdl-card__supporting-text">
					<form method="post" action="#" style="padding-left: 1em; padding-right: 1em; padding-top: 4.5em; padding-bottom: 2.5em;">
						...
						<div class="mdl-textfield mdl-js-textfield" style="margin-bottom: 0.5em;">
							<input name="birthdate" class="mdl-textfield__input" required type="date" min="<?php $time2 = new DateTime();
																											$time2->setDate($time2->format('Y'), 1, 1);
																											$newtime2 = $time2->modify('-170 year')->format('Y-m-d');
																											echo $newtime2; ?>" max="<?php $time = new DateTime();
																					$time->setDate($time->format('Y'), 12, 31);
																					$newtime = $time->modify('-12 year')->format('Y-m-d');
																					echo $newtime; ?>">
						</div>
				</div>
